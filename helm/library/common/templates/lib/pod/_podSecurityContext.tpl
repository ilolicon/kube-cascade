...
{{/* Call this template:
{{ include "common.lib.pod.securityContext" (dict "rootCtx" $ "objectData" $objectData) }}
rootCtx: The root context of the chart.
objectData: The object data to be used to render the Pod.
*/}}
{{- define "common.lib.pod.securityContext" -}}
  {{- $rootCtx := .rootCtx -}}
  ...
  {{- $gpu := (include "common.lib.pod.resources.hasGPU" (dict "rootCtx" $rootCtx "objectData" $objectData)) -}}
  ...
  {{- $hostNet := (eq (include "common.lib.pod.hostNetwork" (dict "rootCtx" $rootCtx "objectData" $objectData)) "true") -}}
  {{- $hostPID := (eq (include "common.lib.pod.hostPID" (dict "rootCtx" $rootCtx "objectData" $objectData)) "true") -}}
  {{- $hostIPC := (eq (include "common.lib.pod.hostIPC" (dict "rootCtx" $rootCtx "objectData" $objectData)) "true") -}}
  {{- if or $hostIPC $hostNet $hostPID -}}
    {{- $hostUsers = true -}}
  {{- end }}

  {{- range $containerName, $containerValues := $objectData.podSpec.containers -}}
    {{- $secContContainer := fromJson (include "common.lib.container.securityContext.calculate" (dict "rootCtx" $rootCtx "objectData" $containerValues)) }}
  ...
  {{- $portRange := fromJson (include "common.lib.helpers.securityContext.getPortRange" (dict "rootCtx" $rootCtx "objectData" $objectData)) -}}
  ...
  {{- if and $portRange.low (le (int $portRange.low) 1024) -}}
    ...
    {{- if ne (include "common.lib.pod.hostNetwork" (dict "rootCtx" $rootCtx "objectData" $objectData)) "true" -}}
  ...
  {{- end }}
fsGroup: {{ include "common.helper.makeIntOrNoop" $secContext.fsGroup }}
fsGroupChangePolicy: {{ $secContext.fsGroupChangePolicy }}
  {{- with $secContext.supplementalGroups }}
supplementalGroups:
    {{- range . }}
  - {{ include "common.helper.makeIntOrNoop" . }}
    {{- end -}}
  {{- else }}
supplementalGroups: []
  {{- end -}}
