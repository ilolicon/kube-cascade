...
{{/* Call this template:
{{ include "common.lib.pod.volume.projected" (dict "rootCtx" $ "objectData" $objectData) }}
rootCtx: The root context of the chart.
objectData: The object data to be used to render the volume.
*/}}
{{- define "common.lib.pod.volume.projected" -}}
  {{- $rootCtx := .rootCtx -}}
  ...
  projected:
    ...
    {{- range $source := $objectData.sources -}}
      ...
      {{- if eq $k "serviceAccountToken" }}
        {{- include "common.lib.pod.volume.projected.serviceAccountToken" (dict "rootCtx" $rootCtx "source" $v) | nindent 6 }}
      {{- else if or (eq $k "secret") (eq $k "configMap") }}
        {{- include "common.lib.pod.volume.projected.cm-secret" (dict "rootCtx" $rootCtx "source" $v "type" $k) | nindent 6 }}
      {{- else if eq $k "downwardAPI" }}
        {{- include "common.lib.pod.volume.projected.downwardAPI" (dict "rootCtx" $rootCtx "source" $v) | nindent 6 }}
      {{- else if eq $k "clusterTrustBundle" }}
        {{- include "common.lib.pod.volume.projected.clusterTrustBundle" (dict "rootCtx" $rootCtx "source" $v) | nindent 6 }}
      {{- else -}}
...
{{- define "common.lib.pod.volume.projected.serviceAccountToken" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $source := .source -}}
...
{{- define "common.lib.pod.volume.projected.downwardAPI" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $source := .source -}}
...
{{- define "common.lib.pod.volume.projected.cm-secret" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $source := .source -}}
  {{- $type := .type -}}
...
{{- define "common.lib.pod.volume.projected.clusterTrustBundle" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $source := .source -}}
