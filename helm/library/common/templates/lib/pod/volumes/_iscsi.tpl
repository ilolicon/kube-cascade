...
{{/* Call this template:
{{ include "common.lib.pod.volume.iscsi" (dict "rootCtx" $ "objectData" $objectData) }}
rootCtx: The root context of the chart.
objectData: The object data to be used to render the volume.
*/}}
{{- define "common.lib.pod.volume.iscsi" -}}
  {{- $rootCtx := .rootCtx -}}
...
- name: {{ $objectData.shortName }}
  iscsi:
    ...
    lun: {{ include "common.helper.makeIntOrNoop" $lun }}
    ...
    {{- if or $authSession $authDiscovery -}}
      {{- $secretName := (printf "%s-%s" (include "common.lib.chart.names.fullname" $rootCtx) $objectData.shortName) }}
    secretRef:
        name: {{ $secretName }}
    {{- end -}}
{{- end -}}
