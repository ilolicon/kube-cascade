...
{{- define "common.lib.container.fixedEnv" -}}
  ...
  {{- $nvidia := false -}}
  {{- if eq (include "common.lib.container.resources.hasGPU" (dict "rootCtx" $rootCtx "objectData" $objectData "gpuType" "nvidia.com/gpu")) "true" -}}
    {{- $nvidia = true -}}
  {{- end -}}
...
- name: {{ $env.k | quote }}
  value: {{ (include "common.helper.makeIntOrNoop" $env.v) | quote }}
  {{- end -}}
{{- end -}}
