...
{{- define "common.lib.container.securityContext.calculate" -}}
  ...
  {{- range $persistenceName, $persistenceValues := $rootCtx.Values.persistence -}}
    ...
    {{- if (eq $enabled "true") -}}
      {{- if eq $persistenceValues.type "device" -}}
        {{- $volume := (fromJson (include "common.lib.container.volumeMount.isSelected" (dict "persistenceName" $persistenceName "persistenceValues" $persistenceValues "objectData" $objectData "key" "persistence"))) -}}
  ...
  {{- $portRange := fromJson (include "common.lib.helpers.securityContext.getPortRange" (dict "rootCtx" $rootCtx "objectData" $tempObjectData)) -}}
