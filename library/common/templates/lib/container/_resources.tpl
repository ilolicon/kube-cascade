...
{{- define "common.lib.resources.validation.data" -}}
...
{{- define "common.lib.container.resources.validation" -}}
  {{- $resources := .resources -}}
  {{- $data := (include "common.lib.resources.validation.data" .) | fromJson -}}
  {{- $regex := $data.regex -}}
...
{{- define "common.lib.pod.resources.hasGPU" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $objectData := .objectData -}}
  {{- $gpuType := .gpuType -}}
  ...
  {{- if $objectData.podSpec -}}
    {{- range $k, $v := $objectData.podSpec.containers -}}
      ...
      {{- range $t := $types -}}
        {{- if eq (include "common.lib.container.resources.hasGPU" (dict "rootCtx" $rootCtx "objectData" $v "gpuType" $t)) "true" -}}
          {{- $gpu = true -}}
          {{- break -}}
        {{- end -}}
      {{- end -}}
...
{{- define "common.lib.container.resources.hasGPU" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $objectData := .objectData -}}
  {{- $gpuType := .gpuType -}}
