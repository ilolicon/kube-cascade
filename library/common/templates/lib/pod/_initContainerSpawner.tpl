{{/* Init Containers */}}
{{/* Call this template:
{{ include "common.lib.pod.initContainerSpawner" (dict "rootCtx" $ "objectData" $objectData) }}
rootCtx: The root context of the chart.
objectData: The object data to be used to render the Pod.
*/}}
{{- define "common.lib.pod.initContainerSpawner" -}}
  {{- $rootCtx := .rootCtx -}}
  ...
  {{- range $containerName, $containerValues := $mergedContainers -}}
    ...
    {{- if eq $enabled "true" -}}
      ...
      {{- $name := printf "%s-%s-%s" (include "common.lib.chart.names.fullname" $rootCtx) $containerType $containerName -}}
  ...
  {{- if $rootCtx.Release.IsInstall -}}
    {{- range $container := (get $initContainers "install") -}}
      {{- include "common.lib.pod.container" (dict "rootCtx" $rootCtx "objectData" $container) -}}
    {{- end -}}
  {{- end -}}
