{{/* Returns Volumes */}}
{{/* Call this template:
{{ include "common.lib.pod.volumes" (dict "rootCtx" $ "objectData" $objectData) }}
rootCtx: The root context of the chart.
objectData: The object data to be used to render the Pod.
*/}}
{{- define "common.lib.pod.volumes" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $objectData := .objectData -}}

  {{- $selectedVolumes := (include "common.lib.pod.volumes.selected" (dict "rootCtx" $rootCtx "objectData" $objectData)) | fromJson -}}

  {{- range $type, $volumes := $selectedVolumes -}}
    {{- range $volume := $volumes -}}
      {{- include (printf "common.lib.pod.volume.%s" $type) (dict "rootCtx" $rootCtx "objectData" $volume) | trim | nindent 0 -}}
    {{- end -}}
  {{- end -}}
{{- end -}}

{{- define "common.lib.pod.volumes.checkRWO" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $objectData := .objectData -}}
  ...
  {{- if (mustHas $type $typesWithAccessMode) -}}
    {{- $modes := include "common.lib.pvc.accessModes" (dict "rootCtx" $rootCtx
        "objectData" $persistence "caller" "Volumes") | fromYamlArray
    -}}

    {{- $hasRWO := include "common.lib.pod.volumes.hasRWO" (dict "modes" $modes) -}}
...
{{- define "common.lib.pod.volumes.hasRWO" -}}
  {{- $modes := .modes -}}
  {{- $hasRWO := false -}}
  {{- range $m := $modes -}}
...
{{- define "common.lib.pod.volumes.selected" -}}
  {{- $rootCtx := .rootCtx -}}
  {{- $objectData := .objectData -}}
